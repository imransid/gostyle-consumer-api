...
A sender implements a single method:
    ...
    send(identifier, code, channel) -> None

The router picks a concrete sender per call: ConsoleSender when OTP_SENDER is
"console" (local dev), otherwise WhatsApp for the phone channel and email for
the email channel. Phone OTP goes over WhatsApp only; there is no SMS fallback.
"""
...
class ConsoleSender:
    ...
    def send(self, identifier, code, channel):
        print(f"\n>>> OTP for {identifier} [{channel}]: {code}\n")
...
def get_sender(channel):
    """Return the sender to use for `channel` given the current configuration."""
    ...
    raise ValueError(f"No sender for channel {channel!r}")
