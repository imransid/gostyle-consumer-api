...
class Seams:
    ...
    def seams(self, *, group=None, series=None):
        ...
        series_fakes = {
            "create_series_booking": mock.Mock(return_value=(201, json.loads(json.dumps(HUB)))),
    ...
    def sent(self, s):
        """What booking-api was sent: (body, keyword arguments)."""
...
class CreateTests(Seams, SimpleTestCase):
    ...
    def test_a_dry_run_without_a_time_sends_the_days_and_converts_the_free_times(self):
        answer = (200, json.loads(json.dumps(PREVIEW_NO_TIME)))
        with self.seams(series={"create_series_booking": mock.Mock(return_value=answer)}) as s:
        ...
        # booking-api's 10:00, 18:30 and 21:55 are the salon's 08:00, 16:30, 19:55.
        self.assertEqual(response.data["available_times"], ["08:00", "16:30", "19:55"])

    def test_a_dry_run_with_a_time_converts_sessions_and_alternatives(self):
        answer = (200, json.loads(json.dumps(PREVIEW)))
        with self.seams(series={"create_series_booking": mock.Mock(return_value=answer)}):
    ...
    def test_without_a_key_the_same_create_carries_the_same_derived_key(self):
        with self.seams() as s:
            self.post(routine())
            self.post(routine())
        keys = [c.kwargs["idempotency_key"] for c in s.create_series_booking.call_args_list]
    ...
    def test_a_refusal_comes_back_as_booking_api_sent_it(self):
        refusal = {"detail": "Please correct the highlighted fields.", "code": "validation_error",
                   "errors": [{"field": "total", "code": "amount_mismatch",
                               ...
                               "expected": 336}]}
        with self.seams(series={"create_series_booking": mock.Mock(return_value=(422, refusal))}):
            response = self.post(routine())
    ...
    def test_a_busy_session_409_comes_back_as_it_is(self):
        ...
        with self.seams(series={"create_series_booking": mock.Mock(return_value=(409, refusal))}):
            response = self.post(routine())
